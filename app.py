...
with tab1:
  ...
  sub_menu = st.selectbox("Select a  subject", data['subject'].unique().tolist())
  
  if menu == 'Popularity':
     data_filter = data[data['subject']==sub_menu] 
     num = st.number_input('Enter a Number you want to be recommended', min_value=5)
     popularidad = popularity_based_recommendation(data_filter, num)
     fig = px.bar(popularidad, y='course_title', x='popularity_score', title='Recomendation Base on Popularity Scores {}'.format(sub_menu))
     st.plotly_chart(fig) 
     ...
     #viz
     num = st.number_input('Enter a Number you want to be recommended', min_value=5)
     w = data_filter[['course_title','subject' ,'weighted_average']].sort_values(by = 'weighted_average', ascending=False).head(num)
